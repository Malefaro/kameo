...
#[allow(missing_debug_implementations)]
#[must_use = "request won't be sent without awaiting, or calling a send method"]
pub struct ReplyRecipientTellRequest<'a, M, R, Tm>
where
    M: Send + 'static,
    R: Response,
{
...
impl<'a, M, R, Tm> ReplyRecipientTellRequest<'a, M, R, Tm>
where
    M: Send + 'static,
    R: Response,
{
    ...
    pub fn mailbox_timeout(
        self,
        duration: Duration,
    ) -> ReplyRecipientTellRequest<'a, M, R, WithRequestTimeout> {
        self.mailbox_timeout_opt(Some(duration))
    }
...
impl<M, R> ReplyRecipientTellRequest<'_, M, R, WithoutRequestTimeout>
where
    M: Send + 'static,
    R: Response,
{
...
impl<'a, M, R, Tm> IntoFuture for ReplyRecipientTellRequest<'a, M, R, Tm>
where
    M: Send + 'static,
    R: Response,
    Tm: Into<Option<Duration>> + Send + 'static,
